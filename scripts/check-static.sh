...
install_command="curl -fsSL https://raw.githubusercontent.com/LineZero-Studio/openclaw-diy/v0.1.0/install.sh | bash"
skip_model_command="curl -fsSL https://raw.githubusercontent.com/LineZero-Studio/openclaw-diy/v0.1.0/install.sh | bash -s -- --skip-model"
telegram_command="curl -fsSL https://raw.githubusercontent.com/LineZero-Studio/openclaw-diy/v0.1.0/scripts/add-telegram.sh | bash"
telegram_status_command="sudo -u openclaw -H bash -lc 'openclaw channels status --channel telegram --probe --json'"
...
for path in "index.html" "README.md"; do
  ...
  assert_contains "$path" "openclaw devices approve"
...
assert_contains "site-config.js" "releaseTag = \"v0.1.0\""
...
# Do not fetch the raw GitHub install URL in CI before release; the v0.1.0 tag
